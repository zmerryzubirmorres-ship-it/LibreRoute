...
Первая тестовая сборка для `arm64-v8a` доступна в
[GitHub Releases](https://github.com/zmerryzubirmorres-ship-it/LibreRoute/releases).
Она подписана локальным отладочным ключом Android, устанавливается как
«LibreRoute Dev» и не предназначена для постоянного канала обновлений.
Не устанавливайте APK исходного проекта: это другая сборка. Проверка
обновлений в приложении обращается к Releases этого репозитория (настройка
`openfluxUpdateRepository` в `gradle.properties`) и открывает страницу
загрузки в браузере; автоматическая установка не реализована. Первый выпуск
помечен как pre-release, поэтому API `/releases/latest`, используемый
приложением, его не возвращает: для этого теста APK нужно скачать вручную.

В этом репозитории находится исходный код Android-клиента. Архив исходников
изменённого Go-ядра OpenFlux и готовые нативные библиотеки приложены к
соответствующему Release. APK без отдельно собранного
`libopenflux_client.so` не является рабочим клиентом.
...
Сначала распакуйте архив исходников Go-ядра из Release в соседний каталог
`LibreRoute-Core` и соберите его (Go 1.26.8) через
